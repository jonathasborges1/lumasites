        ...
        <nav className="hidden lg:flex items-center gap-5 xl:gap-7">
          <div ref={dropdownRef} className="relative">
            <button
              ...
              className={`flex items-center gap-1 font-display text-xs uppercase tracking-[0.18em] xl:tracking-[0.22em] transition-colors ${
                isServicePage
                  ? "text-glow-aqua"
                    ...
                    <Link
                      ...
                      className={`group block border px-4 py-3 transition-colors duration-200 ${
                        pathname === item.href
                          ...
                          : "border-transparent text-ink-primary hover:border-glow-cyan/30 hover:bg-glow-cyan/[0.10] hover:text-glow-aqua"
                      }`}
                    >
                      <span className="block font-display text-xs uppercase tracking-[0.16em] text-current">
                        {item.label}
                      </span>
                      <span className="mt-1.5 block text-xs leading-relaxed text-ink-secondary transition-colors duration-200 group-hover:text-ink-primary">
                        {item.description}
                      </span>
                    </Link>
                  ))}
          ...
          {mainNavigation.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`font-display text-xs uppercase tracking-[0.18em] xl:tracking-[0.22em] transition-colors ${
                pathname === item.href
                  ? "text-glow-aqua"
                  : "text-ink-secondary hover:text-glow-aqua"
              }`}
