#!/usr/bin/env python3
"""
Scraper com browser real (Playwright) — Dr. Juraci Nunes
Abre Chromium, navega no Instagram e extrai identidade visual completa.
Uso: python scripts/scraper_juracinunes_browser.py
     python scripts/scraper_juracinunes_browser.py --login USUARIO SENHA
"""
...
def run(login_user: str = "", login_pass: str = "") -> dict:
    ...
    # Encerra processos Brave em execução (necessário para abrir o perfil com Playwright)
    print("[INFO] Encerrando processos Brave existentes…")
    subprocess.run(["taskkill", "/F", "/IM", "brave.exe"], capture_output=True)
    time.sleep(2)

    # Copia o perfil Default para um diretório temporário e remove o singleton lock
    tmp_dir = Path(tempfile.mkdtemp(prefix="brave_juraci_"))
    ...
    # Copia apenas os arquivos essenciais de sessão (evita copiar GBs desnecessários)
    ...
    with sync_playwright() as p:
        ...
        # Extrair links de posts do grid — apenas do perfil alvo
        post_links = []
        try:
        ...
        # Fallback: qualquer post/reel visível filtrando pelo username
        if not post_links:
            try:
        ...
        for i, href in enumerate(post_links[:12], 1):
            ...
            try:
                ...
                # Screenshot do post (substitui download de CDN — funciona sem cookies no requests)
                ...
                # Imagem — tenta baixar via requests com cookies do browser injetados no header
                try:
                    ...
                    if img_url:
                        ...
                        # Usa cookies da sessão do Playwright para o download
        ...
        # Contagem de cores por frequência
        if all_post_colors:
...
if __name__ == "__main__":
    ...
    print(f"[START] Scraper Juraci Nunes — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    result = run(login_user, login_pass)

    json_path = OUTPUT_DIR / "juracinunes_browser_data.json"
